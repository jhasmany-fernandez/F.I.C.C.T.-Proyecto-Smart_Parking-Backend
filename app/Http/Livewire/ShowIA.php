...
class ShowIA extends Component
{
    ...
    {
        $pythonScript = "helpers/Colores/recognizing-colors-ia.py";
        $process = new Process(['python', $pythonScript]);
        $process->run();
        if (!$process->isSuccessful()) {
            throw new ProcessFailedException($process);
        ...
        if ($result == false) {
            ...
            $body = [
                'notification' => [
                    'body' => '¡Se solicita que estacione bien su vehículo!',
                    'title' => 'Nos hemos dado cuenta que ha parqueado mal su vehículo, estacionelo bien porfavor.'
                ],
                'to' => 'e7TtAb_tQVejao8XOwnqUL:APA91bEXLJKNIkHfFRepxx06EBuGObu7tlFbo0PSxrRTX2CtYrFeTnkbetvcSfUI7cQZZPq7NQsIfEdT8czEfYmsDj9Rb3tfJ0F4fKVBNTlpi9NLIzor4vi-k5OShTyCimQmyqf3tgff'
            ];
            $header = [
    ...
    public function spacesVerification()
    {
        $pythonScript = "helpers/ParkingLot/parking_lot/main.py";
        $process = new Process(['python', $pythonScript]);
        $process->run();
        if (!$process->isSuccessful()) {
            throw new ProcessFailedException($process);
        ...
        dd($result);
        $numero = intval($result);
        if($numero!=1){
            $url = 'https://fcm.googleapis.com/fcm/send';
            $body = [
                'notification' => [
                    'body' => '¡Se solicita que estacione en el lugar de reserva que usted eligió!',
                    'title' => 'Nos hemos dado cuenta que ha parqueado mal su vehículo, estacionelo en el lugar correcto.'
                ],
                'to' => 'e7TtAb_tQVejao8XOwnqUL:APA91bEXLJKNIkHfFRepxx06EBuGObu7tlFbo0PSxrRTX2CtYrFeTnkbetvcSfUI7cQZZPq7NQsIfEdT8czEfYmsDj9Rb3tfJ0F4fKVBNTlpi9NLIzor4vi-k5OShTyCimQmyqf3tgff'
            ];
            $header = [
